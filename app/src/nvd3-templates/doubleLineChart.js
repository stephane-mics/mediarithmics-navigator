...
nv.models.doubleLineChart = function() {
  ...
  function findMinMax(data) {
    ...
    data.forEach(
      function (serie, i) {
      maxTmp = d3.max(serie.values, function (o) {return o.y;} );
      max =  d3.max([max, maxTmp]);

      minTmp = d3.min(serie.values, function (o) {return o.y;} );
      min =  d3.min([min, minTmp]);
      var maxXTmp = d3.max(serie.values, function (o) {return o.x;} );
      var minXTmp = d3.min(serie.values, function (o) {return o.x;} );
    ...
    return {min: Math.floor(min), max: Math.ceil(max), xRange: [minX, maxX],
      allPositive: function(){ return min >= 0 && max >= 0;},
      allNegative: function(){ return min < 0 && max < 0;},
      ratio: function() {return max / (max - min);}
    };
  }

  function rescale(bounds) {
    var scaleMax = Math.floor(Math.log(bounds.max)/Math.LN10);
    var max = comprehensibleValues.filter(function(elem) { return elem * Math.pow(10, scaleMax) >= bounds.max; })[0] * Math.pow(10, scaleMax);
    var neg = false;
    if(bounds.min < 0 ){
      neg = true;
    }
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      var dataLeft = data.filter(function(d) { return !d.disabled && !d.right; });
      var dataRight = data.filter(function(d) { return d.right;  }); // removed the !d.disabled clause here to fix Issue #240
      ...
      .datum(data.filter(function(d) { return !d.disabled  && !d.right;}));
      ...
      dispatch.on('tooltipShow', function(e) {
        if (tooltips && !useInteractiveGuideline) {showTooltip(e, that.parentNode);}
      });
  ...
  //------------------------------------------------------------
  // TODO Find how to fix the ghost tooltip in interactive mode
